...
export async function POST(req: NextRequest) {
  try {
    ...
    // If we have userId but no email, or vice versa, ensure consistency
    if (userId && !email) {
      // Try to get email from User by userId
      try {
        ...
        console.warn('[PRINCIPAL] Failed to lookup user by userId', { userId, error: err });
      }
    }
    
    // If we have email but no userId, ensure User exists and get userId
    if (email && !userId) {
      try {
        console.log('[PRINCIPAL] Resolving userId from email', { email });
        const user = await ensureAssociateMinimal(email);
        userId = user.id;
        if (!userCode && user.code) {
          userCode = user.code;
        }
        console.log('[PRINCIPAL] Resolved userId from email', { userId, email, code: userCode });
      } catch (associateErr: any) {
        console.error('[PRINCIPAL] Failed to resolve userId from email', {
          error: associateErr?.message,
          email,
        });
        // Don't block checkout - continue without userId
      }
    }
