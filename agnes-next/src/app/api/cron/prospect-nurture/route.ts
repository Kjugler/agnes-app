...
export async function GET(req: NextRequest) {
  ...
  if (!adminKey) {
    return NextResponse.json(
      { ok: false, error: 'ADMIN_KEY not configured on agnes-next' },
      { status: 500 },
    );
  }

  const url = `${API_BASE_URL.replace(/\/$/, '')}/api/admin/jobs/send-prospect-nurture`;
  try {
    ...
    console.error('[cron/prospect-nurture]', err);
    return NextResponse.json(
