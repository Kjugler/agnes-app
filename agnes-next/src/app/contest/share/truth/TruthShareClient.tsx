...
export default function TruthShareClient() {
  ...
  const handleDownloadVideo = () => setHasDownloaded(true);
  ...
  const handlePostedConfirmation = async () => {
    ...
    try {
      const res = await fetch('/api/points/award', {
        ...
        body: JSON.stringify({ action: 'share_truth', source: 'truth_share_page' }),
      });
      if (res.ok) {
        setPointsAwarded(true);
        setTimeout(() => router.push('/contest/score'), 1500);
      } else {
  ...
  return (
    ...
    >
      ...
      <h1 style={{ fontSize: '2rem', fontWeight: 600, color: 'white', marginTop: '1rem', marginBottom: '0.5rem', textAlign: 'center' }}>
        Share on Truth Social
      </h1>
      <p style={{ fontSize: '0.875rem', color: '#94a3b8', marginBottom: '2rem', textAlign: 'center', maxWidth: 640 }}>
        Download the video, copy your caption, and post it to Truth Social to earn points.
      </p>

      {truthVideoSrc && (
        <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'center', width: '100%', maxWidth: 640 }}>
          <video
            src={truthVideoSrc}
            controls
            style={{ borderRadius: 12, boxShadow: '0 20px 60px rgba(0,0,0,0.5)', width: '100%', maxWidth: '320px', height: 'auto' }}
          />
        </div>
      )}

      <div style={{ marginTop: '1rem', width: '100%', maxWidth: 640 }}>
        <p style={{ fontWeight: 600, marginBottom: '0.5rem', color: 'white', fontSize: '1rem' }}>Your Truth Social Caption</p>
        <textarea
          readOnly
      ...
      <div style={{ marginTop: '1.5rem', width: '100%', maxWidth: 640, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        {truthVideoSrc && (
          <a
            href={truthVideoSrc}
            download
      ...
      <div style={{ marginTop: '1.5rem', width: '100%', maxWidth: 640, border: '1px solid rgba(148, 163, 184, 0.3)', borderRadius: 8, padding: '1rem', background: 'rgba(15, 23, 42, 0.5)' }}>
        <button
          type="button"
          onClick={() => setShowSteps((prev) => !prev)}
          style={{ width: '100%', textAlign: 'left', fontWeight: 600, background: 'transparent', border: 'none', color: 'white', cursor: 'pointer', fontSize: '1rem', padding: 0 }}
        >
          {showSteps ? 'Hide Truth Social Instructions ▲' : 'How to Post on Truth Social (Step-by-Step) ▼'}
        </button>
        {showSteps && (
          <ol style={{ listStyleType: 'decimal', marginLeft: '1.25rem', marginTop: '0.75rem', color: '#cbd5e1', fontSize: '0.875rem', lineHeight: 1.8, paddingLeft: 0 }}>
            <li style={{ marginBottom: '0.5rem' }}>Tap &quot;Download Truth Video&quot; above and save it to your device.</li>
            <li style={{ marginBottom: '0.5rem' }}>Tap &quot;Open Truth Social to Post&quot; (or open your Truth Social app).</li>
            <li style={{ marginBottom: '0.5rem' }}>Tap the <strong>+</strong> button to create a new post.</li>
            <li style={{ marginBottom: '0.5rem' }}>Select the downloaded video from your camera roll / gallery.</li>
            <li style={{ marginBottom: '0.5rem' }}>Trim or adjust if needed, then tap <strong>Next</strong>.</li>
            <li style={{ marginBottom: '0.5rem' }}>Tap and hold in the caption box, then choose <strong>Paste</strong>.</li>
            <li style={{ marginBottom: '0.5rem' }}>Review your caption and hashtags.</li>
            <li style={{ marginBottom: '0.5rem' }}>Tap <strong>Post</strong>.</li>
            <li style={{ marginBottom: '0.5rem' }}>Come back here and tap &quot;I Posted It&quot; to claim your points.</li>
          </ol>
        )}
      </div>
      ...
      <div style={{ marginTop: '1.5rem', width: '100%', maxWidth: 640 }}>
        <button
          ...
          style={{
            ...
            borderRadius: 999,
            border: 'none',
            background: pointsAwarded ? '#10b981' : awardingPoints ? '#64748b' : '#10b981',
            color: 'white',
            fontSize: '1.1rem',
            fontWeight: 700,
        ...
        >
          {awardingPoints ? 'Recording your share...' : pointsAwarded ? '✓ Points Awarded! Redirecting...' : 'I Posted It ✔ Get My Points'}
        </button>
      </div>

      <JodyAssistant variant="truth" autoShowDelayMs={4000} onShowTraining={() => setShowTruthTraining(true)} />
      ...
      {showTruthTraining && (
        <JodyTrainingModal
          ...
          importantNote={<> <strong>Important:</strong> if the video upload fails or you close the window too quickly, the post may not actually go live. DeepQuill LLC reserves the right to audit winning entries to confirm posts were truly published. We don&apos;t want you to lose eligibility because one video didn&apos;t finish uploading. </>}
          afterPostNote={<> Once Truth Social confirms that your post is live, come back to this page and click <strong>&ldquo;I Posted It ✔ Get My Points&rdquo;</strong>. That records your share in the contest and updates your score on the scoreboard. </>}
        />
      )}
      <HelpButton />
    </div>
