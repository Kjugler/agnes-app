...
function InstructionsContent() {
  ...
  const shareUrl = buildTrackingLink('ig', variant, refCode, target, baseUrl);
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        console.warn('[share/ig/instructions] Failed to fetch user info', err);
      }
    };
    fetchUserInfo();
  }, []);
  ...
  const handleDownloadVideoAgain = () => {
    const href = `/api/share/ig/video?variant=${variant}`;
    const link = document.createElement('a');
    link.href = href;
    link.download = `agnes-protocol-ig-${variant}.mp4`;
  ...
  const backHref = `/share/ig/${variant}${backParams.toString() ? `?${backParams.toString()}` : ''}`;
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '1.5rem', textAlign: 'center' }}>
        Post to Instagram (about 60 seconds)
      </h1>

      {/* Section 1: Training Video */}
      <div
        style={{
          marginBottom: '2rem',
      ...
      >
        ...
        <div style={{ padding: '1rem', background: '#f9fafb', fontSize: '0.875rem', color: '#6b7280', lineHeight: 1.5 }}>
          Step 1: Tap Save… • Step 2: Open Files → Downloads • Step 3: Share to Instagram Reel
        </div>
      </div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        {activeTab === 'ios' ? (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            ...
            <li>Select <strong>Instagram</strong></li>
            <li>Choose <strong>Reel</strong></li>
            <li>Tap <strong>Next</strong></li>
            <li>Press and hold caption area → <strong>Paste</strong></li>
            <li>Post</li>
            <li>App switcher → return to browser → tap I Posted ✅</li>
          </ol>
        ) : (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            <li>Confirm download (swipe down → tap notification or open Downloads)</li>
      ...
      {/* Section 3: Buttons (mirror TikTok: Copy Caption first, then Download) */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: 400, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: '#9ca3af', textAlign: 'center', marginTop: '1.5rem', lineHeight: 1.5 }}>
          ...
          <Link href="/posted?platform=ig" style={{ color: '#3b82f6', fontWeight: 600, textDecoration: 'underline' }}>
            /posted?platform=ig
          </Link>
          .
        </p>
...
export default function InstagramInstructionsPage() {
  return (
    <Suspense
      fallback={
        <div
