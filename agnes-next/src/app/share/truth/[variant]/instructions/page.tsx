...
function InstructionsContent() {
  ...
  const shareUrl = buildTrackingLink('truth', variant, refCode, target, baseUrl);
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        console.warn('[share/truth/instructions] Failed to fetch user info', err);
      }
    };
    fetchUserInfo();
  }, []);
  ...
  const handleDownloadVideoAgain = () => {
    const href = `/api/share/truth/video?variant=${variant}`;
    const link = document.createElement('a');
    link.href = href;
    link.download = `agnes-protocol-truth-${variant}.mp4`;
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '1.5rem', textAlign: 'center' }}>
        Post to Truth Social (about 60 seconds)
      </h1>

      {/* Section 1: Training Video Placeholder */}
      <div
        style={{
          marginBottom: '2rem',
      ...
      >
        <div style={{ padding: '0.75rem 1rem', background: '#f9fafb', fontWeight: 600, fontSize: '1rem' }}>
          Training Video (coming next)
        </div>
        <div
          style={{
            aspectRatio: '16/9',
        ...
        >
          Jody tutorial will appear here
        </div>
        <div style={{ padding: '1rem', background: '#f9fafb', fontSize: '0.875rem', color: '#6b7280', lineHeight: 1.5 }}>
          Placeholder narration (script will be added later)
        </div>
      </div>

      {/* Section 2: Steps (device-aware) */}
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ fontSize: '0.95rem', fontWeight: 700, color: '#1a1a1a', marginBottom: '1rem', lineHeight: 1.5 }}>
          <strong>IMPORTANT: Post to your feed (do not send as a message).</strong>
        </p>

        {activeTab === 'ios' ? (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            <li>Confirm download (tap Save… if you see it at the bottom of Safari)</li>
            <li>Open <strong>Files</strong> → <strong>Browse</strong> → <strong>Downloads</strong></li>
            <li>Tap the Agnes video file</li>
            <li>Tap the <strong>Share</strong> icon (box with arrow up)</li>
            <li>Choose <strong>Truth Social</strong></li>
            <li>In Truth Social composer: Ensure it&apos;s creating a <strong>Post</strong> (not message)</li>
            <li>Attach video if not already attached</li>
            <li>Press and hold in the text area → <strong>Paste</strong> caption</li>
            <li>Tap <strong>Post</strong> / Share</li>
            <li>App switcher → return to browser → tap I Posted ✅</li>
          </ol>
        ) : (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            <li>Confirm download (swipe down → tap download OR open Downloads)</li>
            <li>Open <strong>Files</strong> → <strong>Downloads</strong></li>
            <li>Find the Agnes video file</li>
            <li>Tap three dots → <strong>Share</strong></li>
            <li>Select <strong>Truth Social</strong></li>
            <li>In Truth Social composer: Ensure it&apos;s creating a <strong>Post</strong></li>
            <li>Press &amp; hold → <strong>Paste</strong> caption</li>
            <li>Tap <strong>Post</strong></li>
            <li>Recent Apps → return to browser → tap I Posted ✅</li>
          </ol>
        )}
      </div>
...
export default function TruthInstructionsPage() {
  return (
    <Suspense
      fallback={
        <div
