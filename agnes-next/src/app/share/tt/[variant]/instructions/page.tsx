...
function InstructionsContent() {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        console.warn('[share/tt/instructions] Failed to fetch user info', err);
      }
    };
    fetchUserInfo();
  }, []);
  ...
  const handleDownloadVideoAgain = () => {
    const href = `/api/share/tt/video?variant=${variant}`;
  ...
  const backHref = `/share/tt/${variant}${backParams.toString() ? `?${backParams.toString()}` : ''}`;
  ...
  return (
    ...
    >
      <h1 style={{ fontSize: '1.75rem', fontWeight: 800, marginBottom: '1.5rem', textAlign: 'center' }}>
        Post to TikTok (about 60 seconds)
      </h1>
      ...
      >
        ...
        <div style={{ padding: '1rem', background: '#f9fafb', fontSize: '0.875rem', color: '#6b7280', lineHeight: 1.5 }}>
          Step 1: Tap Save… • Step 2: Open Files → Downloads • Step 3: Share to TikTok
        </div>
      </div>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        {activeTab === 'ios' ? (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            <li>Look at the bottom bar in Safari → tap Save…</li>
            ...
            <li>Tap <strong>TikTok</strong></li>
            <li>Press &amp; hold in TikTok caption → <strong>Paste</strong></li>
            <li>Post ✅</li>
          </ol>
        ) : (
          <ol style={{ margin: 0, paddingLeft: '1.5rem', lineHeight: 2, fontSize: '1rem' }}>
            <li>Swipe down → tap the download notification (or open Downloads from your browser menu)</li>
            <li>Tap the Agnes video</li>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: 400, margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '0.85rem', color: '#9ca3af', textAlign: 'center', marginTop: '1.5rem', lineHeight: 1.5 }}>
          ...
          <Link href="/posted" style={{ color: '#3b82f6', fontWeight: 600, textDecoration: 'underline' }}>
            /posted
          </Link>
          .
        </p>
...
export default function TikTokInstructionsPage() {
  return (
    <Suspense
      fallback={
        <div
