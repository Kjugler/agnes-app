...
export default function SignalDetailClient({ signal }: { signal: SignalData }) {
  ...
  const tags: string[] = Array.isArray(signal.tags)
    ? signal.tags.filter((t): t is string => typeof t === 'string')
    : [];
  ...
  const handleDeleteSignal = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // After deletion, keep UX consistent by returning to the feed.
        router.push('/signal-room');
        return;
      }
      ...
      alert(errorMsg || 'Failed to delete');
    } catch (err) {
