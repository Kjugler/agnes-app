          ...
          >
            <div style={{ flex: 1, minWidth: 200 }}>
              ...
              <div style={{ fontSize: '0.85em', color: '#888' }}>
                {s.type || 'NARRATIVE'} • {s.publishStatus || 'PUBLISHED'}
                {s.discussionEnabled ? ' • Discussion on' : ' • Discussion off'}
                {s._count && ` • ${s._count.comments} comments`}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              <Link
                ...
                style={{ color: '#00ffe0', fontSize: '0.85em' }}
              >
                View
              </Link>
              <button
              ...
              {s.publishStatus === 'DRAFT' && (
                <button
                  ...
                  onClick={async () => {
                    try {
                      const res = await fetch(`/api/admin/signals/${s.id}/publish`, {
                        method: 'POST',
