...
async function handleAssociateStatus(req, res) {
  try {
    ...
    let user = null;
    /** True after we already ran findUnique by id for contest_user_id cookie (avoids duplicate id query). */
    let userIdLookupDoneForCookie = false;

    // Resolve by userId cookie (canonical) — one User query with full select (previously two sequential queries).
    if (userIdCookie) {
      userIdLookupDoneForCookie = true;
      try {
    ...
    if (!user && userId && !userIdLookupDoneForCookie) {
      user = await prisma.user.findUnique({
        where: { id: userId },
        select,
      });
