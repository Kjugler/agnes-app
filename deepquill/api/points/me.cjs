...
const { ensureRabbitState } = require('../../lib/rabbit.cjs');
...
async function handlePointsMe(req, res) {
  try {
    ...
    // Rabbit progression state (canonical) - ensure target/seq correct for canonical total
    let rabbitTarget = user.rabbitTarget ?? null;
    let rabbitSeq = user.rabbitSeq ?? 1;
    let nextRankThreshold = 500;
    try {
      const { user: rabbitUser, nextRankThreshold: nrt } = await ensureRabbitState(
        prisma,
        { id: user.id, points: user.points, rabbitTarget: user.rabbitTarget, rabbitSeq: user.rabbitSeq },
        calculatedTotalPoints
      );
      rabbitTarget = rabbitUser.rabbitTarget ?? null;
      rabbitSeq = rabbitUser.rabbitSeq ?? 1;
      nextRankThreshold = nrt;
    } catch (rabbitErr) {
      console.warn('[points/me] Failed to ensure rabbit state', {
        error: rabbitErr.message,
        userId: user.id,
      });
    }
