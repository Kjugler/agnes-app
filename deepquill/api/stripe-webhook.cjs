        ...
        case 'checkout.session.completed': {
          ...
          if (paymentStatus === 'paid') {
                  ...
                  if (buyerUser && !existingPurchase.userId) {
                    ...
                    if (email && existingPurchase.id) {
                      try {
                        ...
                        // Order (canonical fulfillment): create for physical products only
                        if (product === 'paperback' && fulfillmentPrisma) {
                          try {
                            const orderData = {
                              customerId: customer.id,
                              ...
                              currency: session.currency || null,
                              contestPlayerId: buyerUser?.id || null,
                              referralCode: session.metadata?.referralCode || session.metadata?.ref || null,
                              shippingName: name ?? null,
                              shippingAddressLine1: addr?.line1 ?? null,
                            ...
                            await fulfillmentPrisma.order.upsert({
                              where: { stripeSessionId: session.id },
                              update: orderData,
                              create: orderData,
                            });
                            console.log('[ORDER] ✅ upserted (paperback)', { stripeSessionId: session.id, customerId: customer.id });
                          } catch (orderErr) {
                            console.error('[ORDER] ❌ Failed to upsert Order', { error: orderErr.message, stripeSessionId: session.id });
                            // Don't fail webhook - Order is for fulfillment UX
                          }
                        }
            ...
            try {
              ...
              if (prisma && buyerUser?.id) {
                try {
                  const rollup = await getPointsRollupForUser(prisma, buyerUser.id);
                  buyerTotalPoints = rollup.totalPoints;
                  console.log('[WEBHOOK] Email: using ledger rollup totalPoints', {
                    buyerId: buyerUser.id,
                    totalPoints: buyerTotalPoints,
                    note: 'includes just-awarded purchase points',
                  });
                } catch (rollupErr) {
                  console.warn('[WEBHOOK] Failed to get rollup for email, falling back to user.points', {
                  ...
                  if (buyerWithPoints) buyerTotalPoints = buyerWithPoints.points;
                }
              } else if (prisma && customerEmail && !buyerUser?.id) {
                try {
                  const normalizedEmail = normalizeEmail(customerEmail);
                  if (normalizedEmail) {
                    ...
                    if (buyerByEmail) {
                      const rollup = await getPointsRollupForUser(prisma, buyerByEmail.id);
                      buyerTotalPoints = rollup.totalPoints;
                    }
                  }
                } catch (e) {
                  console.warn('[WEBHOOK] Fallback points lookup failed', { error: e?.message });
                }
              }
