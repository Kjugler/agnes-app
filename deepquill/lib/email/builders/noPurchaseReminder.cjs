// deepquill/lib/email/builders/noPurchaseReminder.cjs
// Port from agnes-next noPurchaseReminder.ts

function buildNoPurchaseReminderEmail({ firstName, buyUrl, referUrl, shareUrl, journalUrl }) {
  const subjects = [
  ...
  const html = `
    ...
    <h3>🌅 Imagine this:</h3>
    <p>Your kids racing down the hallway of a cruise ship…<br/>Your spouse smiling — the real one, not the "we're managing" one…<br/>You, leaning over the balcony railing at sunset, thinking:<br/><strong>I made this happen.</strong></p>
    <p>And you can.</p>
    ...
    <p>👉 <a href="${shareUrl}"><strong>Share your invite link</strong></a><br/>Help your friends join the challenge. (More participants → more rewards → more fun.)</p>
    <p>👉 <a href="${referUrl}"><strong>Refer a friend</strong></a><br/>Earn extra entries the moment they sign up.</p>
    <p>👉 <a href="${journalUrl}"><strong>Write in your journal</strong></a><br/>What are you hoping for? Who would you take on that vacation?<br/>Your "why" shapes your journey.</p>
    ...
    <p>Stay steady.<br/>Stay hopeful.<br/>You're closer than you think.</p>
    <p>—Vector 🛰️<br/>DeepQuill LLC</p>
  `;
  return { subject, html };
}

module.exports = { buildNoPurchaseReminderEmail };
